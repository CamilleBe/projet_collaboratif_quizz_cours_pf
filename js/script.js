...
// Écouteur d'événement pour le clic sur le bouton de démarrage du quiz
startQuizButton.addEventListener('click', () => {
    // Vérification si le champ de pseudo est vide
    if (pseudoInput.value.trim() === '') {
        // Affichage d'une alerte si le champ de pseudo est vide
        alert('Veuillez entrer un pseudo.');
        return; // Sortie de la fonction si le champ de pseudo est vide
    }

    // Lecture de la musique de fond et gestion des erreurs
    backgroundMusic.play().catch(e => {
        console.error("La lecture automatique de la musique a échoué", e);
    });

    // Affichage du conteneur du quiz en utilisant Flexbox
    quizContainer.style.display = 'flex';

    // Masquage des éléments de démarrage du quiz
    ...
    if (questions && questions.length > 0) {
        // Démarrage du quiz si des questions sont disponibles
        startQuiz();
    }
});
